...
if __name__ == "__main__":
	...
	for i in data.get_other_point():
		...
		MODEL.addConstr(gurobipy.quicksum(x[j] for j in out_edges) <= 1000 * gurobipy.quicksum(x[j] for j in in_edges))
		# MODEL.addConstr((gurobipy.quicksum(x[j] for j in in_edges) == 0) >> (gurobipy.quicksum(x[j] for j in out_edges) == 0))
		MODEL.addConstr(1 >= gurobipy.quicksum(x[j] for j in in_edges))
		
	u = MODEL.addVars(data.get_point_number(), lb=0, ub=data.get_point_number() - 1, vtype=gurobipy.GRB.INTEGER)
	for i in range(data.get_line_number()):
		first_component = data.get_first_component_per_line(i)
		MODEL.addConstr(u[first_component] == 0)
	...
	w = MODEL.addVars(data.get_point_number(), lb=0, ub=data.get_line_number(), vtype=gurobipy.GRB.INTEGER)
	for i in range(data.get_line_number()):
		components_position = data.get_all_component_per_line(i)
		for component in components_position:
			MODEL.addConstr(w[component] == i + 1)
			
	for i in range(data.get_edge_number()):
		pre_point = data.get_index_to_edge()[i][0]
		next_point = data.get_index_to_edge()[i][1]
		MODEL.addConstr((x[i] == 1) >> (w[pre_point] == w[next_point]))
		# MODEL.addConstr(w[pre_point] - w[next_point] <= 100 * (1 - x[i]))
		# MODEL.addConstr(w[next_point] - w[pre_point] <= 100 * (1 - x[i]))
