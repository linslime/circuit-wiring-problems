...
if __name__ == "__main__":
	...
	other_point = set([i for i in range(data.get_point_number())]) - set([data.get_first_component_per_line(0)])
	for i in other_point:
		in_edges = data.get_point_to_in_edge()[i]
		MODEL.addConstr(gurobipy.quicksum(x[j] for j in in_edges) == 1)

	u = MODEL.addVars(data.get_point_number(), lb=0, ub=gurobipy.GRB.INFINITY, vtype=gurobipy.GRB.INTEGER)
	first_component = data.get_first_component_per_line(0)
