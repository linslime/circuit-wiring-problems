...
if __name__ == "__main__":
	...
	first_component = data.get_first_component_per_line(0)
	...
	for i in data.get_residual_component_per_line(0):
		in_edges = data.get_point_to_in_edge()[i]
		MODEL.addConstr(gurobipy.quicksum(x[j] for j in in_edges) == 1)
	
	other_point = set([i for i in range(data.get_point_number())]) - set(data.get_all_component_per_line(0))
	
	for i in other_point:
		if i not in data.get_point_to_out_edge().keys():
			continue
		...
		MODEL.addConstr(gurobipy.quicksum(x[j] for j in out_edges) <= 100 * gurobipy.quicksum(x[j] for j in in_edges))
	...
	# MODEL.setObjective(gurobipy.quicksum(x[i] for i in range(data.get_edge_number())), gurobipy.GRB.MAXIMIZE)
